...
import { DialogButton } from "@/app/components/Dialog";
const Page = () => {
  useEffect(() => {
  ...
  // Example usage:
  // sendEmail('recipient@example.com', 'Welcome to the Course', 'https://example.com/course-link',);
        ...
        <div className="flex flex-col-reverse lg:flex-row justify-between gap-20">
          <div className="w-[100%] lg:w-7/12">
            ...
            <div className="flex flex-col mt-6 gap-5">
              {foundObjectById?.description.map((desc, i) => (
                <p className="text-slate-600 text-lg" key={i}>
                  {desc}
                </p>
              ))}
            </div>
            ...
            {courseSlug == "business-analysis" && (
              <div className="mt-7 grid gap-3 w-full sm:inline-flex flex-wrap">
                {foundObjectById?.enrollWithoutInternship && (
                  <a
            ...
            {foundObjectById?.tools && (
              <div className="flex mt-8 flex-wrap gap-2 mb-0">
                {foundObjectById?.tools?.map((tool, i) => (
                  <span
                    key={i}
                    className={`inline-flex items-center gap-x-1.5 py-2 px-5 rounded-full text-[16px] font-medium ${tool.color} dark:bg-teal-800/30 dark:text-teal-500`}
                  >
                    {tool.tool}
                  </span>
                ))}
          ...
          <div className="w-[100%] lg:w-6/12 lg:max-h-[24.5rem] overflow-hidden">
            <img
              src={foundObjectById?.thumbnail}
              className="rounded-xl lg:h-[27.5rem]"
            />
          </div>
        </div>
        {foundObjectById?.notes && (
          <div className=" mt-16 lg:mt-32 flex flex-col lg:flex-row justify-between gap-16 ">
            <div className="lg:w-6/12">
              <h1 className="text-2xl  text-greenPrimary">
                {foundObjectById?.title} Track without career bootcamp{" "}
              </h1>
              <div className="mt-6">
                {foundObjectById?.notes?.map((note, i) => (
                  <p key={i} className="flex  items-center gap-4 text-lg">
                    <span className="text-greenPrimary text-2xl">.</span> {note}{" "}
                  </p>
                ))}
                

                <div className="w-[100%] h-20"></div>
                <p className="flex  items-center gap-4 text-xl mt-10">Pricing:
                  <span className="ml-2 text-2xl">
                    {foundObjectById?.amountWithoutInternship}
                  </span>
                </p>
                <div className="mt-7 grid gap-3 w-full sm:inline-flex flex-wrap">
                  {foundObjectById?.enrollWithoutInternship && (
...
<a
                  ...
                  {/* <div className="">
                    <DialogButton
                      course={foundObjectById.title}
                      link={foundObjectById?.brochureWithInternship}
                    />
                  </div> */}
                  {/* <a
                    className="inline-flex justify-center items-center gap-x-3 text-center bg-greenPrimary hover:bg-greenSecondary border border-transparent text-sm lg:text-base hover:text-white text-white font-medium rounded-md focus:outline-none focus:ring-2 focus:ring-greenPrimary focus:ring-offset-2 focus:ring-offset-white transition py-3 px-4 dark:focus:ring-offset-gray-800"
            ...
            {foundObjectById.amountWithInternship && (
              <div className="lg:w-6/12">
                <h1 className="text-2xl  text-greenPrimary">
                  {foundObjectById?.title} Track <br /> with career bootcamp{" "}
                </h1>
                <div className="mt-6">
                  {foundObjectById?.notes?.map((note, i) => (
                    <p key={i} className="flex  items-center gap-4 text-lg">
                      <span className="text-greenPrimary text-2xl">.</span>{" "}
                      {note}{" "}
                    </p>
                  ))}

                  <p className="flex h-20  items- gap-4 text-lg">
                    <span className="text-greenPrimary text-2xl">.</span>
                    ⁠Career Bootcamp : {"    "} daily/weekly data analytics
                    opportunities, <br /> ⁠career coaching, ⁠bi-weekly interview and
                    resume writing sessions
                  </p>

                  {/* <p className="flex  items-center gap-4 text-lg pl-10">
                    Daily/weekly Data Analytics Opportunities
                  </p>
                  <p className="flex  items-center gap-4 text-lg pl-10">
                    ⁠Career Coaching
                  </p>
                  <p className="flex  items-center gap-4 text-lg pl-10">
                    ⁠Bi-weekly interview and resume writing sessions
                  </p> */}

                  <p className="flex  items-center gap-4 text-xl mt-10">
                    Pricing:
                    <span className="ml-2 text-2xl">
                      {foundObjectById?.amountWithInternship}
                    </span>
                  ...
                  <div className="mt-7 grid gap-3 w-full sm:inline-flex flex-wrap">
                    {foundObjectById?.enrollWithInternship && (
                      <a
                        className="inline-flex justify-center items-center gap-x-3 text-center bg-greenPrimary hover:bg-greenSecondary border border-transparent text-sm lg:text-base hover:text-white text-white font-medium rounded-md focus:outline-none focus:ring-2 focus:ring-greenPrimary focus:ring-offset-2 focus:ring-offset-white transition py-3 px-4 dark:focus:ring-offset-gray-800"
        ...
        <div className="mt-20 max-w-[60rem] mx-auto flex flex-col gap-5">
          {foundObjectById?.curriculum &&
            foundObjectById.curriculum.map((item, i) => (
              <div key={i} className="h-fit overflow-x-hidden">
                <CourseAccordion
                  index={i}
                  title={item.title}
                  week={item.week}
